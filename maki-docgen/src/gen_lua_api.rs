...
mod tests {
    ...
    fn generated_docs_contain_typed_arguments_contract() {
        ...
        let required = [
            ...
            "For legacy commands, `opts.fargs` remains the existing",
            ...
            "use only the custom provider",
            "combine defaults and custom",
            "Custom candidates replace duplicate",
            "Providers return",
            "values without command-line quotes.",
            "navigation = \"directory\"",
            "callbacks for the provider that owns",
            "Session cancellation calls `on_cancel` once",
            "Accept calls `on_accept` for the",
            "Callbacks stay bound to",
            "The registration owns callback cleanup",
